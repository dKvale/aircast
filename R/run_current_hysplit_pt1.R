...
#"C:\Users\dkvale\Documents\R\R-3.5.2\bin\i386\Rscript.exe" --no-save --no-restore "X:\Agency_Files\Outcomes\Risk_Eval_Air_Mod\_Air_Risk_Evaluation\Staff folders\Dorian\AQI\aircast\R\run_current_hysplit_pt1.R"
...
if (!min_exists(paste0(Sys.Date(), "_AQI_raw_HYSPLIT2.csv"), min_size = 100)) {
...
aqi_traj <- function(date            = NULL, 
  ...
  for (site in unique(sites$site_catid)) {
    ...
    traj    <-  hysplit_traj(lat          = round(site_df$monitor_lat, 3),
                             ...
                             vert_motion  = 0,
                             model_height = 20000,
                             traj_name    = as.character(1000000*round(runif(1), 6)),
                             met_files    = met_list) 
...
met_list      <- paste0("__today/", 
                        c("hysplit.t12z.namf", "hysplit.t12z.nama", 
                          "hysplit.t06z.namf", "hysplit.t06z.nama"))
...
back_forecast <- bind_rows(back_forecast, 
                           aqi_traj(date = today, receptor_height = elev_ht, traj_hours = 24))
...
write_csv(back_forecast, paste0(Sys.Date() - days_past, "_AQI_raw_HYSPLIT_pt1.csv"))
