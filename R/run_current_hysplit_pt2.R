...
setwd("~")
setwd("../Desktop/aircast/hysplit/_pt2")
...
aqi_traj <- function(date            = NULL, 
  ...
  for (site in unique(sites$site_catid)) {
    ...
    traj    <-  hysplit_traj(lat          = round(site_df$monitor_lat, 3),
                             ...
                             direction    = "backward",
                             met_type     = "NAM",
                             met_dir      = "C:/Users/dkvale/Desktop/aircast/hysplit/_pt2/",
                             extended_met = TRUE,
                             vert_motion  = 0,
                             model_height = 20000,
                             traj_name    = paste0("pt2", as.character(1000000*round(runif(1), 6))),
                             met_files    = met_list) 
...
met_list      <- paste0("../__today/", 
                        c("hysplit.t12z.namf", "hysplit.t12z.nama", "hysplit.t06z.namf", "hysplit.t06z.nama"))

# Drop missing met data
met_list      <- met_list[min_exists(met_list)]

back_forecast <- aqi_traj(date = today + 2, receptor_height = 10, traj_hours = 48)

back_forecast <- bind_rows(back_forecast, 
                           aqi_traj(date = today + 2, receptor_height = elev_ht, traj_hours = 48))
...
write_csv(back_forecast, paste0(Sys.Date() - days_past, "_AQI_raw_HYSPLIT_pt2.csv"))
