...
if (ncol(yest_board) > 8) {
  ...
  yest_board <- yest_board %>% 
                mutate_at(vars(`O3 on target %`:`O3 yellow accuracy %`, `PM on target %`:`PM yellow accuracy %`), round)
  
  yest_board$`O3 error ppb` <- round(yest_board$`O3 error ppb`, 1)
  ...
  yest_board <- yest_board %>% 
                mutate_at(vars(`PM on target %`:`PM yellow accuracy %`), round)
...
if (ozone_season) {
  ...
  yest_board <- formattable(select(yest_board[-6, ], Model, `PM error ug/m3`:`PM yellow accuracy %`),
                            list(Model                   = margin_right(24),
                                 `PM error ug/m3`        = margin_right(100),
                                 `PM on target %`        = margin_right(110),
                                 `PM bullseyes %`        = margin_right(114),
