...
# Date for report
report_date <- Sys.Date() - 1 - days_past
...
`r format(report_date, "%A %b %d, %Y")`</p>  
...
# Test for weekend
weekend <- weekdays(report_date) %in% c("Sunday","Monday")


# Test for Ozone season
ozone_season <- yday(report_date) > 74 & month(report_date) %in% 3:9
...
#-- Dates
fcast_date <- report_date - as.numeric(gsub("day", "", verify$forecast_day[1]))
...
if(nrow(verify_o3) < 1) {
  ...
  verify_o3$background_color <- as.character(NA)
...
lead_board <- try(read_csv(paste0("X:/Agency_Files/Outcomes/Risk_Eval_Air_Mod/_Air_Risk_Evaluation/Staff folders/Dorian/AQI/Verification/Archive leaders/", report_date, "-lead_board.csv")),
                  silent = TRUE)
...
yest_board <- bind_cols(select(yest_board, matches("O3 err|M error|Model")) %>%
                         ...
                         select(yest_board, -matches("O3 err|M error|Model")) %>%
                           mutate_if(is.numeric, round)) %>%
              select(Model, contains("O3"), contains("PM"))

# Replace NA's with spaces
yest_board <- mutate_all(yest_board, as.character)
...
lead_board <- lead_board %>%
              mutate(`Yellow accuracy %` = ifelse(is.na(`Yellow accuracy %`), " ", as.character(`Yellow accuracy %`)))
...
yest_board <- subset(yest_board, !Model %in% c("Farmer Weeks", "Persistant Assistant", "J.K. Rollings", "BB3 Blender",  ""))
```
...
lead_board <- subset(lead_board, !Model %in% c("Farmer Weeks", "Persistant Assistant", "J.K. Rollings", "BB3 Blender",  ""))
...
formattable::formattable(pm_board[  , 1:ncol(pm_board)])
```
...
if (ozone_season) {
  ...
  formattable::formattable(o3_board[  , 1:ncol(o3_board)])
}
```
