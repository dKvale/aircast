...
# C: & "C:\Users\dkvale\Documents\R\R-3.4.0\bin\i386\Rscript.exe" --no-save --no-restore "~\..\Desktop\aircast\aircast\R\email_background.R"
...
verify <- left_join(select(verify, -short_name), 
...
names(verify_pm)[4:20] <-  c("Ozone obs count", "Ozone ppb", "Ozone AQI", "Ozone forecast", 
                             "CMAQ forecast",
                             "persist_ozone_aqi", "roll_ozone_aqi", "roll11_ozone_aqi", "bb3_ozone_aqi",
                             "PM2.5 obs count", "PM2.5 ug/m3", "PM2.5 AQI", "PM2.5 forecast",
                             "persist_pm25_aqi", "roll_pm25_aqi", "roll11_pm25_aqi", "bb3_pm25_aqi")
...
names(verify_o3)[4:20] <-  c("Ozone obs count", "Ozone ppb", "Ozone AQI", "Ozone forecast", 
                             "CMAQ forecast",
                             "persist_ozone_aqi", "roll_ozone_aqi", "roll11_ozone_aqi", "bb3_ozone_aqi",
                             "PM2.5 obs count", "PM2.5 ug/m3", "PM2.5 AQI", "PM2.5 forecast",
                             "persist_pm25_aqi", "roll_pm25_aqi", "roll11_pm25_aqi", "bb3_pm25_aqi")
...
yest_board <- data_frame(Model                    = c("Staniel Dirwin", "Dr Robot", "GradBooster", "Big RF", "Ensemble",  "CMAQ", "Farmer Weeks", "Persistant Assistant","J.K. Rollings", "BB3 (aka 'Blender')"),
                        `new O3 accuracy`         = o3_performance$accuracy[c(5,7,8,6,9:10,1,3,2,11)],
                        `new O3 yellow accuracy`  = o3_performance$elev_accuracy[c(5,7,8,6,9:10,1,3,2,11)],
                        `new O3 error ppb`        = o3_performance$mean_conc_diff[c(5,7,8,6,9:10,1,3,2,11)],
                        `new O3 bullseyes`        = o3_performance$bulls_eyes[c(5,7,8,6,9:10,1,3,2,11)],
                        `new O3 on target`        = o3_performance$near_misses[c(5,7,8,6,9:10,1,3,2,11)],
                        `new PM accuracy`         = pm_performance$accuracy[c(5,7,8,6,9,NaN,1,3,2,10)],
                        `new PM yellow accuracy`  = pm_performance$elev_accuracy[c(5,7,8,6,9,NaN,1,3,2,10)],
                        `new PM error ug/m3`      = pm_performance$mean_conc_diff[c(5,7,8,6,9,NaN,1,3,2,10)],
                        `new PM bullseyes`        = pm_performance$bulls_eyes[c(5,7,8,6,9,NaN,1,3,2,10)],
                        `new PM on target`        = pm_performance$near_misses[c(5,7,8,6,9,NaN,1,3,2,10)],
                         new_obs                  = 1)
...
if (!weekend) {
  ...
  if (!ozone_season) {
     ...
     if (is.na(yest_board[lead_board$Model == "CMAQ", ]$`new O3 error ppb`)) lead_board$new_obs[6] <- 0
...
yest_board <- select(yest_board, Model, 
                     `new O3 error ppb`, `new O3 on target`, `new O3 bullseyes`, `new O3 accuracy`, `new O3 yellow accuracy`,   
                     `new PM error ug/m3`, `new PM on target`, `new PM bullseyes`, `new PM accuracy`, `new PM yellow accuracy` )
...
names(yest_board) <- c("Model",
                        "O3 error ppb", "O3 on target %", "O3 bullseyes %", "O3 cat accuracy %",  "O3 yellow accuracy %", 
                        "PM error ug/m3", "PM on target %", "PM bullseyes %", "PM cat accuracy %", "PM yellow accuracy %")


yest_board <- yest_board %>% mutate_at(vars(`O3 error ppb`:`O3 yellow accuracy %`, `PM on target %`:`PM yellow accuracy %`), round)


yest_board$`PM error ug/m3` <- round(yest_board$`PM error ug/m3`, 1)
...
yest_board[yest_board$Model == "CMAQ", c("PM error ug/m3", "PM on target %", "PM bullseyes %", "PM cat accuracy %", "PM yellow accuracy %")] <- " "
...
if (ozone_season) {
  ...
  yest_board <- formattable(select(yest_board[-6, ], -c(`O3 error ppb`:`O3 yellow accuracy %`)),
                            list(Model                   = margin_right(24),
                                 `PM error ug/m3`        = margin_right(100),
                                 `PM on target %`        = margin_right(110),
                                 `PM bullseyes %`        = margin_right(114),
...
names(yest_board) <- gsub("PM ", "", names(yest_board))
names(yest_board) <- gsub("bull", "Bull", names(yest_board))
names(yest_board) <- gsub("cat", "CAT", names(yest_board))
names(yest_board) <- gsub("on", "On", names(yest_board))
names(yest_board) <- gsub("yell", "Yell", names(yest_board))
names(yest_board) <- gsub("err", "Err", names(yest_board))
...
lead_board <- select(lead_board, c(Model, `O3 error ppb`, `O3 on target %`, `O3 bullseyes %`, `O3 cat accuracy %`, `O3 yellow accuracy %`,
                                   `PM error ug/m3`), `PM on target %`, `PM bullseyes %`, `PM cat accuracy %`, `PM yellow accuracy %`, `Forecasts n`, `O3 Forecasts n`)
...
lead_board$`O3 yellow accuracy %` <- as.numeric(lead_board$`O3 yellow accuracy %`)

lead_board <- lead_board %>% 
                mutate(`O3 error ppb`         = `O3 error ppb` %>% as.numeric() %>% round(), 
...
# Blank CMAQ for PM2.5 forecasts
lead_board[lead_board$Model == "CMAQ", c("PM error ug/m3", "PM on target %", "PM bullseyes %", "PM cat accuracy %", "PM yellow accuracy %")] <- " "


# Blank NAs for days without yellow/orange
lead_board[is.na(lead_board$`O3 yellow accuracy %`), "O3 yellow accuracy %"] <- " "
...
if (ozone_season) {
  ...
  lead_board <- formattable(lead_board,
                            list(Model                   = margin_right(24),
                                 ...
                                 `PM cat accuracy %`     = margin_right(110),
                                 `PM yellow accuracy %`  = margin_right(124)),
                            align = c("l"), digits = 1, format = "html")

} else{
  
  lead_board <- formattable(select(lead_board[-6, ], -c(`O3 error ppb`:`O3 yellow accuracy %`)),
                            list(Model                   = margin_right(24),
                                 `PM on target %`        = margin_right(110),
                                 `PM cat accuracy %`     = margin_right(110),
                                 `PM yellow accuracy %`  = margin_right(124),
...
verify_all <- select(verify_all, Site:`MPCA forecast`, mod_aqi_ens:mod_aqi_rf, `CMAQ forecast`, pollutant:background_color, `Obs conc`:`MPCA conc`, `Model output`:mod_conc_gen1, `CMAQ conc`)

names(verify_all) <- c("Site", "Group", "Obs count", "Obs AQI", 
                       "MPCA AQI", "Ensemble AQI", "GradBoost AQI", "Robot AQI", 
                       "Big RF AQI", "CMAQ AQI", "pollutant", "background_color", 
                       "Monitored", "MPCA", "Ensemble", "Big RF", 
                       "GradBoost", "Robot", "CMAQ")
...
yest_board[1, 1] <- "Stan (MPCA MET Team)"
yest_board[ifelse(ozone_season, 10, 9), 1] <- "BB3 (the Blender)"
...
if (ozone_season) {
  ...
  yest_board[yest_board$Model != "CMAQ", ]
...
if (ozone_season) {
  ...
  yest_board[yest_board$Model != "CMAQ", c(1, 7:11)]
...
lead_board[1, 1] <- "Stan (MPCA MET Team)" 
lead_board[ifelse(ozone_season, 10, 9), 1] <- "BB3 (the Blender)"
